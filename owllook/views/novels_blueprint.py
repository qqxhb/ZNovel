...
async def index(request):
    ...
    if user:
        return template('index.html', title='bupt - 快读小说搜索引擎', is_login=1, user=user,
                        search_ranking=search_ranking[:25])
    else:
        return template('index.html', title='bupt - 快读小说搜索引擎', is_login=0, search_ranking=search_ranking[:25])
...
async def owllook_search(request):
    ...
    result_sorted = obj.Get_Data_By_Body(doc, param)
    #print(result_sorted)
    if result_sorted:
        ...
        total = int(result_sorted["total"])/10
        if int(result_sorted["total"]) % 10 != 0:
            total = total+1
        user = request['session'].get('user', None)
        search_ranking = await cache_owllook_search_ranking()
        if user:
            ...
            return template(
                ...
                books=result_sorted['hits'],
                count=result_sorted["total"],
                current=current,
                total=total,
